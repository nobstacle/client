...
export default function HeaderOnlyPage() {
  ...
  // FINAL WORKING LISTENER – receives real user from extension
  useEffect(() => {
    const handler = (event: MessageEvent) => {
      ...
      try {
        // Decode JWT without verifying (we trust the extension read the real cookie)
        const payload = JSON.parse(atob(token.split('.')[1]));
        const user = payload.user || payload; // NextAuth v4 structure

        console.log('REAL USER FROM EXTENSION:', user);
  ...
  const isLoading = isInIframe ? extensionAuthStatus === 'loading' : status === 'loading';
  const isAuthenticated = isInIframe ? extensionAuthStatus === 'authenticated' : status === 'authenticated';
  const effectiveUser = isInIframe ? { user: extensionUser, expires: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString() } : session;

  console.info("LOADING STATE", isLoading, isAuthenticated);
  
  if (!mounted) return <div>Loading...</div>;
  if (isLoading) {
    return <div style={{ padding: '16px', background: '#fff8e1', textAlign: 'center' }}>Checking login...</div>;
  }
  if (!isAuthenticated || !effectiveUser?.user) {
    return (
      <div style={{ padding: '16px', background: '#f8d7da', textAlign: 'center' }}>
        Not logged in
        <button onClick={() => window.open('https://nobstacle.com', '_blank')}>Login</button>
      </div>
    );
  }
  ...
  return (
    <SocketContextProvider>
      <ClientHeader user={effectiveUser} />
    </SocketContextProvider>
  );
}