...
export const LanguageShortcutPicker = ({
  checkIframe = true,
  ...
  if (!isHydrated) {
    return (
      <div className={`${checkIframe || compactDesktop ? 'h-6 w-24' : 'h-8 w-32'} bg-white/20 rounded animate-pulse`} />
    );
  }
  ...
  if (isLoading && languageShortcuts.length === 0) {
    return (
      <div className={`flex items-center ${checkIframe || compactDesktop ? 'gap-2 px-2' : 'gap-4 px-3'}`}>
        <span className={`text-white/60 ${checkIframe || compactDesktop ? 'text-xs' : 'text-sm'}`}>
          Loading...
        </span>
      </div>
    );
        ...
        {sorted.map((shortcut) => {
          ...
          return (
            <button
              ...
              className={`flex items-center ${dense ? 'gap-1 px-1.5 py-0.5' : 'gap-2 px-3 py-1'} rounded transition-all ${
                isActive
            ...
            >
              <div
                className={`${dense ? 'w-2 h-2' : 'w-3 h-3'} rounded-full border-2 transition-all ${
                  isActive ? "border-white bg-white" : "border-white/60"
                }`}
              >
                {isActive && (
                  <div className={`${dense ? 'w-0.5 h-0.5 m-0.5' : 'w-1.5 h-1.5 m-0.5'} rounded-full bg-[#3b5998]`} />
                )}
              </div>
              <span className={`${dense ? 'text-[11px]' : 'text-sm'} font-medium`}>
